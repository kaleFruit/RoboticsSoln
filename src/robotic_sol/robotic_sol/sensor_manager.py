...
class SensorManager(Node):
    def __init__(self):
        ...
        self.server_addresses = [('127.0.0.3', 10000), ('127.0.0.1', 10000)] #for the two sensors
    ...
    def run_asyncio_loop(self):
        ...
        for idx, socketInfo in enumerate(self.server_addresses):
            loop.create_task(self.async_connect(socketInfo, idx))
        loop.run_forever()

    async def async_connect(self, socketInfo, idx):
        try:
            ...
            while True:
                ...
                self.get_logger().info(f'Received: {data_filtered}')
                self.latest_data[idx] = data.reshape(-1, 3)[-1].tolist()
            sock.close()
