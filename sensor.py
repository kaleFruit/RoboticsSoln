...
class Sensor(Thread):  
    ...
    def __init__(self,
        ...
        self.DOF = 3 #MODIFIED; originally was 6 dof 
    ...
    def recive(self, buffer_size: int) -> int:
        received_data = b'' 
        # modified receiving protocol by adding a delimiter
        while True:
            received_msg = self.connection.recv(buffer_size)
            while not received_msg:
                received_msg = self.connection.recv(buffer_size)
            received_data += received_msg
            while b'\n' in received_data:
                msg, received_data = received_data.split(b'\n', 1)
                msg = msg.decode().strip() 
    
                if msg.isnumeric(): 
                    return int(msg)  
                else:
                    print("Received a non-numeric message")
